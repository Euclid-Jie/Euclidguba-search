...
def save_data(data_df, SaveFolderPath, FilePath):
    """
    轮子函数，用于存储数据，可实现对已存在文件的追加写入
    :param SaveFolderPath: the FordPath save data files, usually is the projectPath: os.getcwd()
    :param data_df: 目标数据
    :param FilePath: 文件名，包括后缀
    :return:
    """
    # concat the folderPath and dataPath
    FileFullPath = os.path.join(SaveFolderPath, FilePath)
...
if __name__ == "__main__":
    col = MongoClient("guba", "中国医药_base")
    with tqdm(range(1, 59)) as t:
        for page in t:
            t.set_description("page:{}".format(page))  # 进度条左边显示信息
            writed_len = get_data(page, col)
